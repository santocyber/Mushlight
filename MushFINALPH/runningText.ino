...
void filltxt(String text, uint32_t color) {
   if (loadingFlag) {
    offset = WIDTHX;   // перемотка в правый край
    loadingFlag = false;    
  ...
  if (scrollTimer.isReady()) {
    ...
    while (text[i] != '\0') {
      if ((byte)text[i] > 191) {    // работаем с русскими буквами!
        ...
        drawLetter2(j, text[i], offset + j * (LET_WIDTH + SPACE), color);
        i++;
        j++;
      }
    }
...
void drawLetter2(uint8_t index, uint8_t letter, int16_t offset, uint32_t color) {
  int8_t start_pos = 0, finish_pos = LET_WIDTH;
  int8_t LH = LET_HEIGHT;
  if (LH > HEIGHT) LH = HEIGHT;
  int8_t offset_y = (HEIGHT - LH) / 2;     // по центру матрицы по высоте
  ...
  for (byte i = start_pos; i < finish_pos; i++) {
    ...
    for (byte j = 0; j < LH; j++) {
      ...
      // рисуем столбец (i - горизонтальная позиция, j - вертикальная)
      if (thisBit) leds[getPixelNumber2(offset + i, offset_y + TEXT_HEIGHT + 0)] = letterColor;
    }
  }
}
